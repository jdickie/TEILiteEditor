...
eXSLTForms.rte.specificFunctions[ 'CKEditor' ].beforeRendering = function( nativeConfigOptionsObject ) {
	...
	eXSLTForms.registry.plugins[ 'teiann'] = {
		...
		loadingPluginModules : function() {
			...
			document.getElementById( 'teiannGeneralConfigOptions' ).appendChild( eXSLTForms.utils.loader( CKEDITOR.basePath + 'plugins/tei-ann/config/generalConfigOptions.xml', 'xml').documentElement );
			generalConfigOptionsDoc = document.getElementById( 'teiannGeneralConfigOptions' );

			//loading the annotator specifications data instance
			...
			pluginLangCode = generalConfigOptionsDoc.querySelector( "lang[code = '" + editorLangCode + "']" ).getAttribute( 'code' ) == null ? 'en' : editorLangCode;
