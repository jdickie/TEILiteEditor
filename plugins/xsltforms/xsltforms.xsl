...
-->
<xsl:stylesheet xmlns:txs="http://www.agencexml.com/txs" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:default="http://www.w3.org/1999/xhtml" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:exslt="http://exslt.org/common" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ajx="http://www.ajaxforms.net/2006/ajx" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0" exclude-result-prefixes="xhtml xforms ev exslt msxsl">
    ...
    <xsl:include href="/rest/db/dma/apps/tei-annotator/utils/exsltforms/exsltforms.xsl"/>		
<!--
...
-->
    ...
    <xsl:template match="xhtml:html | html">
        ...
        <xsl:variable name="lang">
            <xsl:choose>
                <xsl:when test="$xsltforms_lang != ''">
                    <xsl:value-of select="$xsltforms_lang"/>
                </xsl:when>
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            ...
            <xsl:variable name="displaydebug">
                <xsl:choose>
                    <xsl:when test="$xsltforms_debug != ''">
                        <xsl:value-of select="$xsltforms_debug"/>
                    </xsl:when>
            ...
            <head>
                ...
                <script type="text/javascript">
                    ...
                    <xsl:for-each select="//xforms:bind[contains(@type,':')]">
                        ...
                        <xsl:if test="not(preceding::xforms:bind[starts-with(@type,$nstype)])">
                            <xsl:variable name="nsmodel">
                                <xsl:for-each select="//xforms:model[@schema]">
                                    <xsl:value-of select="document(@schema,/)/*[descendant::*[@name = $typename]]/@targetNamespace"/>
                                </xsl:for-each>
                            </xsl:variable>
                            <xsl:variable name="nsuri">
                                <xsl:choose>
                                    <xsl:when test="//namespace::*[name()=$nstype]">
                                        <xsl:value-of select="//namespace::*[name()=$nstype][1]"/>
                                    </xsl:when>
                                    <xsl:when test="//*[starts-with(name(),concat($nstype,':'))]">
                                        <xsl:value-of select="namespace-uri(//*[starts-with(name(),concat($nstype,':'))][1])"/>
                                    </xsl:when>
                                    <xsl:when test="//@*[starts-with(name(),concat($nstype,':'))]">
                                        <xsl:value-of select="namespace-uri(//@*[starts-with(name(),concat($nstype,':'))][1])"/>
                                    </xsl:when>
                                    <xsl:when test="//xsd:schema[descendant::*[@name = $typename]]">
                                        <xsl:value-of select="//xsd:schema[descendant::*[@name = $typename]]/@targetNamespace"/>
                                    </xsl:when>
                                    <xsl:when test="$nsmodel != ''">
                                        <xsl:value-of select="$nsmodel"/>
                                    </xsl:when>
                            ...
                            <xsl:text>Schema.registerPrefix('</xsl:text>
                            <xsl:value-of select="$nstype"/>
                            <xsl:text>', '</xsl:text>
                        ...
                        <xsl:if test="not(preceding::*/@xsi:type[starts-with(.,$nstype)])">
                            <xsl:variable name="nsuri">
                                <xsl:choose>
                                    ...
                                    <xsl:when test="//xforms:model[@schema]">
                                        <xsl:for-each select="//xforms:model[@schema]">
                                            <xsl:value-of select="document(@schema,/)/*[descendant::*[@name = $typename]]/@targetNamespace"/>
                                        </xsl:for-each>
                                    </xsl:when>
                    ...
                    <xsl:variable name="xexprs">
                        <xexprs xmlns="">
                            <xsl:for-each select="//xforms:*/@at | //xforms:*/@calculate | //xforms:*/@constraint | //xforms:*/@context | //xforms:*/@if | //xforms:*/@index | //xforms:*/@nodeset | //xforms:*/@origin | //xforms:*/@readonly | //xforms:*/@ref | //xforms:*/@relevant | //xforms:*/@required | //xforms:*/@target | //xforms:*/@value | //xforms:*/@while">
								...
								-->
                            <xsl:if test="//xforms:bind[not(@nodeset)]">
                                <xexpr>*[1]</xexpr>
                            </xsl:if>
                        </xexprs>
                    </xsl:variable>
                ...
                <xsl:call-template name="eXSLTForms"/>
                <xsl:copy-of select="$config/extensions/afterInit/*"/>
            </head>
            <body>
                <xsl:if test="$xsltforms_caller != 'true'">
                    <xsl:attribute name="onload">init();if (window.xf_user_init) xf_user_init();<xsl:value-of select="$initdebug"/>
                        <xsl:value-of select="xhtml:body/@onload"/>
                        <xsl:value-of select="body/@onload"/>
                    </xsl:attribute>
                </xsl:if>
...
-->
    <xsl:template match="*[@*[contains(.,'{') and contains(substring-after(.,'{'),'}')] and not(@id)]">
        <xsl:copy>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:dialog">
        <div>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:group">
...
-->
    ...
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xhtml:br">
        <xsl:element name="br"/>
    </xsl:template>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:input">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:item">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:itemset">
        <xsl:param name="type" select="false()"/>
        <xsl:choose>
            <xsl:when test="$type">
                <div>
                    ...
                    <div class="xforms-item">
                        <xsl:attribute name="id">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>xf-itemset-item-<xsl:value-of select="count(preceding::xforms:itemset|ancestor::xforms:itemset)"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:attribute>
                        <input type="{$type}" value="{xforms:value}"/>
            ...
            <xsl:otherwise>
                <option class="xforms-disabled">
                    <xsl:call-template name="genid"/>&#160;</option>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:label">
        <xsl:param name="appearance" select="false()"/>
        <xsl:choose>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:message|ajx:confirm">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:output">
        <xsl:param name="appearance" select="false()"/>
        <xsl:call-template name="field">
            <xsl:with-param name="appearance" select="$appearance"/>
            <xsl:with-param name="body">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <span>
                            <xsl:call-template name="comun"/>&#160;<xsl:text/>
                        </span>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:with-param>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:secret">
        <xsl:param name="appearance" select="false()"/>
        <xsl:call-template name="field">
            <xsl:with-param name="appearance" select="$appearance"/>
            <xsl:with-param name="body">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:select1|xforms:select">
        <xsl:param name="appearance" select="false()"/>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:switch">
        <div>
            <xsl:call-template name="genid"/>
            <xsl:call-template name="style">
...
-->
    <xsl:template match="xhtml:table[xforms:repeat] | table[xforms:repeat] | xhtml:thead[xforms:repeat] | thead[xforms:repeat] | xhtml:tbody[xforms:repeat] | tbody[xforms:repeat] | xhtml:tfoot[xforms:repeat] | tfoot[xforms:repeat]">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="ajx:tabs">
        <div>
            <xsl:call-template name="style">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:textarea">
        <xsl:param name="appearance" select="false()"/>
        <xsl:call-template name="field">
            <xsl:with-param name="appearance" select="$appearance"/>
            <xsl:with-param name="body">
...
-->
    <xsl:template match="xhtml:tr[parent::xforms:repeat] | tr[parent::xforms:repeat]">
        <xsl:apply-templates select="node()"/>
    </xsl:template>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:tree">
        <div>
            <xsl:call-template name="style">
            ...
            <ul>
                <li class="xforms-tree-item">
                    <span class="xforms-tree-item-button">
                        <xsl:text/>
                    </span>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xforms:trigger|xforms:submit">
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" name="field">
        <xsl:param name="appearance" select="false()"/>
        <xsl:param name="body"/>
        <span>
            <xsl:call-template name="genid"/>
            <xsl:call-template name="style">
                <xsl:with-param name="class">xforms-control xforms-<xsl:value-of select="local-name()"/>
                    <xsl:choose>
                        <xsl:when test="local-name()='trigger' or local-name()='submit' or string(xforms:label)=''"> xforms-appearance-minimal</xsl:when>
                        <xsl:when test="@appearance"> xforms-appearance-<xsl:value-of select="@appearance"/>
                        </xsl:when>
                        <xsl:otherwise> xforms-appearance</xsl:otherwise>
                    </xsl:choose>
                </xsl:with-param>
            </xsl:call-template>
            <span>
                    ...
                    <span>
                        ...
                        <span class="xforms-alert">
                            ...
                            <xsl:if test="xforms:alert">
                                <xsl:variable name="aid">
                                    <xsl:choose>
                                        <xsl:when test="xforms:alert/@id">
                                            <xsl:value-of select="xforms:alert/@id"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                        ...
                        <xsl:if test="xforms:hint">
                            <span class="xforms-hint">
                                <span class="xforms-hint-icon" onmouseover="show(this, 'hint', true)" onmouseout="show(this, 'hint', false)">&#160;<xsl:text/>
                                </span>
                                <xsl:variable name="hid">
                                    <xsl:choose>
                                        <xsl:when test="xforms:hint/@id">
                                            <xsl:value-of select="xforms:hint/@id"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                        ...
                        <xsl:if test="xforms:help[not(@appearance='minimal')]">
                            <span class="xforms-help">
                                <span class="xforms-help-icon" onmouseover="show(this, 'help', true)" onmouseout="show(this, 'help', false)">&#160;<xsl:text/>
                                </span>
                                <xsl:variable name="hid">
                                    <xsl:choose>
                                        <xsl:when test="xforms:help/@id">
                                            <xsl:value-of select="xforms:help/@id"/>
                                        </xsl:when>
                                        <xsl:otherwise>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" name="group">
...
-->
    <xsl:template name="comunLabel">
        ...
        <xsl:call-template name="genid"/>
        <xsl:call-template name="style">
            <xsl:with-param name="class">xforms-label <xsl:value-of select="$class"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
...
-->
    <xsl:template name="style">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:attribute name="class">
                    <xsl:value-of select="normalize-space(concat(@class, ' xforms-disabled ', $class))"/>
                </xsl:attribute>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="genid">
        ...
        <xsl:attribute name="id">
            <xsl:choose>
                ...
                <xsl:otherwise>xf-<xsl:value-of select="$lname"/>-<xsl:value-of select="count(preceding::*[local-name()=$lname and namespace-uri()=$nsuri]|ancestor::*[local-name()=$lname and namespace-uri()=$nsuri])"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:attribute>
    </xsl:template>
...
-->
    <xsl:template name="toScriptParam">
        ...
        <xsl:choose>
            <xsl:when test="$p">"<xsl:call-template name="toXPathExpr">
...
-->
    <xsl:template name="toScriptBinding">
        ...
        <xsl:choose>
            <xsl:when test="@bind">new Binding(false, null, null, "<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="@bind"/>
                </xsl:call-template>")</xsl:when>
            <xsl:when test="$xpath != '' and $model != ''">new Binding(<xsl:value-of select="$isvalue"/>, "<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="$xpath"/>
                </xsl:call-template>", "<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="$model"/>
                </xsl:call-template>")</xsl:when>
            <xsl:when test="$xpath != ''">new Binding(<xsl:value-of select="$isvalue"/>, "<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="$xpath"/>
                </xsl:call-template>")</xsl:when>
            <xsl:otherwise>null</xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="xpath">
        ...
        <xsl:variable name="xp2jsres2">
            <xsl:choose>
                ...
                <xsl:when test="$xp2jsres != ''">
                    <xsl:value-of select="$xp2jsres"/>
                </xsl:when>
                <xsl:otherwise>"Unrecognized expression '<xsl:value-of select="$xp"/>'"</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="result">XPath.create("<xsl:call-template name="toXPathExpr">
                <xsl:with-param name="p" select="$xp"/>
            </xsl:call-template>",<xsl:value-of select="$xp2jsres2"/>
            <xsl:call-template name="js2ns">
                <xsl:with-param name="js" select="$xp2jsres"/>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:variable name="op">
                                    <xsl:choose>
                                        <xsl:when test="starts-with($p,'div') or starts-with($p,'and') or starts-with($p,'mod')">
                                            <xsl:value-of select="substring($p,1,3)"/>
                                        </xsl:when>
                                        <xsl:when test="starts-with($p,'or') or starts-with($p,'!=') or starts-with($p,'&lt;=') or starts-with($p,'&gt;=')">
                                            <xsl:value-of select="substring($p,1,2)"/>
                                        </xsl:when>
                                    ...
                                    <xsl:when test="$op!='null'">
                                        ...
                                        <xsl:variable name="xp3">
                                            <xsl:choose>
                                                ...
                                                <xsl:otherwise>
                                                    <xsl:value-of select="substring($p,string-length($op)+1)"/>
                                                </xsl:otherwise>
                                            </xsl:choose>
                                        </xsl:variable>
                                        <xsl:call-template name="xp2js">
...
-->
    <xsl:template name="closepar">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="concat(string-length($s),'.',$s,string-length($args),'.',$args,string-length($ops),'.',$ops)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
...
-->
    <xsl:template name="getName">
        ...
        <xsl:variable name="r">
            <xsl:choose>
                <xsl:when test="$o = ''">
                    <xsl:value-of select="substring($s,1,100)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="substring-before($s, substring($o,1,1))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:choose>
...
-->
    <xsl:template name="getLocationPath">
        ...
        <xsl:choose>
            <xsl:when test="$s = ''">
                <xsl:value-of select="concat($l,'.',$r)"/>
            </xsl:when>
            <xsl:otherwise>
                ...
                <xsl:variable name="i">
                    <xsl:choose>
                        ...
                        <xsl:when test="$c = '*' and substring($s2,2,1) != ':'">
                            <xsl:value-of select="$axislength + 1"/>.,new StepExpr('<xsl:value-of select="$axis2"/>',new NodeTestType(NodeType.ELEMENT)</xsl:when>
                        <xsl:when test="$c = '/'">1.</xsl:when>
                        <xsl:when test="$c = '@'">
                            <xsl:variable name="t">
                                <xsl:call-template name="getName">
                                    <xsl:with-param name="s" select="substring($s2,2)"/>
                                </xsl:call-template>
                            </xsl:variable>
                            <xsl:choose>
                                <xsl:when test="$t != ''">
                                    <xsl:variable name="pt">
                                        <xsl:if test="not(contains($t,':'))">:</xsl:if>
                                        <xsl:value-of select="$t"/>
                                    </xsl:variable>
                                    <xsl:value-of select="string-length($t)+1"/>.,new StepExpr('attribute',new NodeTestName(<xsl:choose>
                                        <xsl:when test="starts-with($pt,':')">null</xsl:when>
                                        <xsl:otherwise>'<xsl:value-of select="substring-before($pt,':')"/>'</xsl:otherwise>
                                    </xsl:choose>,'<xsl:value-of select="substring-after($pt,':')"/>
                                    <xsl:text>')</xsl:text>
                                </xsl:when>
                                <xsl:otherwise>
                        ...
                        <xsl:when test="starts-with($s2,'node()')">
                            <xsl:value-of select="$axislength + 6"/>.,new StepExpr('<xsl:value-of select="$axis2"/>',new NodeTestAny()</xsl:when>
                        <xsl:when test="contains('_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',$c)">
                            <xsl:variable name="t">
                                <xsl:call-template name="getName">
                            ...
                            <xsl:value-of select="$axislength + string-length($t)"/>.,new StepExpr('<xsl:value-of select="$axis2"/>',new NodeTestName('<xsl:value-of select="substring-before($pt,':')"/>','<xsl:value-of select="substring-after($pt,':')"/>
                            <xsl:text>')</xsl:text>
                        </xsl:when>
                        <xsl:when test="starts-with($s2,'*:') and contains('_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',substring($s2,3,1))">
                            <xsl:variable name="t">
                                <xsl:call-template name="getName">
                                    <xsl:with-param name="s" select="substring($s2,3)"/>
                                </xsl:call-template>
                            </xsl:variable>
                            <xsl:choose>
                                <xsl:when test="not(contains($t,':'))">
                                    <xsl:value-of select="$axislength + 2 + string-length($t)"/>.,new StepExpr('<xsl:value-of select="$axis2"/>',new NodeTestName('*','<xsl:value-of select="$t"/>
                                    <xsl:text>')</xsl:text>
                                </xsl:when>
                                <xsl:otherwise>
                                ...
                                <xsl:when test="substring($s3,1,1) = '['">
                                    <xsl:variable name="t">
                                        <xsl:call-template name="getPredicates">
                                            <xsl:with-param name="s" select="substring($s3,2)"/>
                                        </xsl:call-template>
                                    </xsl:variable>
...
-->
    <xsl:template name="getPredicates">
        ...
        <xsl:variable name="p">
            <xsl:variable name="t">
                <xsl:call-template name="xp2js">
                    <xsl:with-param name="xp" select="$s"/>
                </xsl:call-template>
            </xsl:variable>
            <xsl:choose>
                ...
                <xsl:when test="$t != ''">
                    <xsl:value-of select="string-length($s)-number(substring-before($t,'.'))+1"/>.,new PredicateExpr(<xsl:value-of select="substring-after($t,'.')"/>
                    <xsl:text>)</xsl:text>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template name="fctfullname">
        ...
        <xsl:choose>
            <xsl:when test="contains($fctname,':')">
                ...
                <xsl:choose>
                    <xsl:when test="($main/descendant::*|$main/descendant::*/@*)/namespace::*[name()=$ns]">
                        <xsl:value-of select="($main/descendant::*|$main/descendant::*/@*)/namespace::*[name()=$ns][1]"/>
                    </xsl:when>
                    <xsl:when test="($main/descendant::*|$main/descendant::*/@*)[starts-with(name(),concat($ns,':'))]">
                        <xsl:value-of select="namespace-uri(($main/descendant::*|$main/descendant::*/@*)[starts-with(name(),concat($ns,':'))][1])"/>
                    </xsl:when>
...
-->
    <xsl:template match="xhtml:link[@type='text/css' and @rel='stylesheet'] | link[@type='text/css' and @rel='stylesheet']">
        <xsl:choose>
...
-->
    <xsl:template xmlns="http://www.w3.org/1999/xhtml" match="xhtml:style | style">
...
-->
    <xsl:template name="addsel">
        ...
        <xsl:variable name="sel">
            <xsl:choose>
                <xsl:when test="contains($sels,' ') and contains($sels,',')">
                    <xsl:value-of select="substring-before(translate($sels,',',' '),' ')"/>
                </xsl:when>
                <xsl:when test="contains($sels,' ')">
                    <xsl:value-of select="substring-before($sels,' ')"/>
                </xsl:when>
                <xsl:when test="contains($sels,',')">
                    <xsl:value-of select="substring-before($sels,',')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$sels"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:choose>
        ...
        <xsl:choose>
            ...
            <xsl:when test="translate(substring($input, 1, 8),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz') = '@import '">
                ...
                <xsl:variable name="url2">
                    <xsl:choose>
                        <xsl:when test="starts-with($url,'url(')">
                            <xsl:value-of select="substring-before(substring-after($url,'url('),')')"/>
                        </xsl:when>
                        <xsl:otherwise>
                ...
                <xsl:call-template name="cssconv">
                    ...
                    <xsl:with-param name="xformscontext" select="$xformscontext"/>
                    <xsl:with-param name="xhtmlcontext">
                        <xsl:value-of select="$xhtmlcontext"/>
                    </xsl:with-param>
                </xsl:call-template>
                <xsl:call-template name="cssconv">
                    ...
                    <xsl:with-param name="xformscontext">
                        <xsl:value-of select="$xformscontext"/>
                    </xsl:with-param>
                    <xsl:with-param name="xhtmlcontext">
                        <xsl:value-of select="$xhtmlcontext"/>
                    </xsl:with-param>
...
-->
    <xsl:template match="xforms:bind" mode="script" priority="1">
        ...
        <xsl:call-template name="toXPathExpr">
            <xsl:with-param name="p" select="$nodeset"/>
        </xsl:call-template>
        <xsl:text>",</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@type"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@readonly"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@required"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@relevant"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@calculate"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@constraint"/>
        </xsl:call-template>
        <xsl:text>);
</xsl:text>
        <xsl:apply-templates select="*" mode="script"/>
...
-->
    <xsl:template match="xforms:delete" mode="script" priority="1">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@at"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
...
-->
    <xsl:template match="xforms:group" mode="script" priority="1">
        ...
        <xsl:call-template name="toScriptBinding">
            <xsl:with-param name="p" select="@ref"/>
        </xsl:call-template>
        <xsl:text>);
</xsl:text>
        <xsl:apply-templates select="*" mode="script"/>
...
-->
    <xsl:template match="xforms:input | xforms:secret | xforms:textarea" mode="script" priority="1">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@inputmode"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@incremental"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@ajx:aid-button"/>
        </xsl:call-template>
        <xsl:text>);
</xsl:text>
        <xsl:apply-templates select="*" mode="script"/>
...
-->
    <xsl:template match="xforms:insert" mode="script" priority="1">
        ...
        <xsl:call-template name="toScriptParam">
            ...
            <xsl:with-param name="default">"after"</xsl:with-param>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@origin"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@context"/>
...
-->
    <xsl:template match="xforms:instance" mode="script" priority="1">
        ...
        <xsl:choose>
            <xsl:when test="@src">
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="@src"/>
                </xsl:call-template>
                <xsl:text>,null);
</xsl:text>
            </xsl:when>
            <xsl:when test="@resource and not(*)">
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="@resource"/>
                </xsl:call-template>
                <xsl:text>,null);
</xsl:text>
            </xsl:when>
...
-->
    <xsl:template match="xforms:item | xforms:itemset[ancestor::xforms:*[1][@appearance='full']]" mode="script" priority="1">
        ...
        <xsl:text>",</xsl:text>
        <xsl:call-template name="toScriptBinding">
            <xsl:with-param name="p" select="xforms:label/@ref"/>
            <xsl:with-param name="model" select="string(xforms:label/@model)"/>
        </xsl:call-template>
        <xsl:if test="xforms:value">
            <xsl:text>,</xsl:text>
            <xsl:call-template name="toScriptBinding">
                <xsl:with-param name="p" select="xforms:value/@ref"/>
                <xsl:with-param name="model" select="string(xforms:value/@model)"/>
            </xsl:call-template>
        </xsl:if>
        <xsl:text>);
</xsl:text>
    ...
    <xsl:template match="xforms:load" mode="script" priority="1">
        ...
        <xsl:choose>
            ...
            <xsl:when test="xforms:resource/text()">
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="normalize-space(xforms:resource/text())"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="toScriptParam">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@show"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
...
-->
    <xsl:template match="xforms:message" mode="script" priority="1">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@level"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
...
-->
    <xsl:template match="xforms:model" mode="script" priority="1">
        ...
        <xsl:text>",</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@schema"/>
        </xsl:call-template>
        <xsl:text>);
</xsl:text>
...
-->
    <xsl:template match="xforms:output" mode="script" priority="1">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@mediatype"/>
        </xsl:call-template>
        <xsl:text>);
</xsl:text>
        <xsl:apply-templates select="*" mode="script"/>
    ...
    <xsl:template match="xforms:reset" mode="script" priority="1">
        <xsl:choose>
            <xsl:when test="parent::xforms:action">
                ...
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="@model"/>
                    <xsl:with-param name="default">xforms.defaultModel</xsl:with-param>
                </xsl:call-template>
                <xsl:text>,</xsl:text>
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="@if"/>
            ...
            <xsl:otherwise>
                ...
                <xsl:text>"),</xsl:text>
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="@ev:event"/>
                </xsl:call-template>
...
-->
    <xsl:template match="xforms:send" mode="script" priority="1">
        ...
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@submission"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
...
-->
    <xsl:template match="xforms:setfocus" mode="script" priority="1">
        ...
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@control"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
...
-->
    <xsl:template match="xforms:setindex" mode="script" priority="1">
        ...
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@repeat"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@index"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
...
-->
    <xsl:template match="xforms:setvalue" mode="script" priority="1">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="normalize-space(text())"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
...
-->
    <xsl:template match="xforms:show | xforms:hide" mode="script" priority="1">
        ...
        <xsl:text>',</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@dialog"/>
        </xsl:call-template>
        <xsl:text>,null,null);
</xsl:text>
        <xsl:apply-templates select="*" mode="script"/>
    ...
    <xsl:template match="xforms:submission" mode="script" priority="1">
        ...
        <xsl:choose>
            <xsl:when test="xforms:resource/@value">
                ...
                <xsl:for-each select="xforms:resource[1]">
                    <xsl:call-template name="toScriptBinding">
                        <xsl:with-param name="model" select="$idmodel"/>
                    </xsl:call-template>
                </xsl:for-each>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="resource">
                    <xsl:choose>
                        <xsl:when test="xforms:resource">
                            <xsl:value-of select="xforms:resource"/>
                        </xsl:when>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="method">
                    <xsl:choose>
                        <xsl:when test="xforms:method">
                            <xsl:value-of select="xforms:method"/>
                        </xsl:when>
                ...
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="$method"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@version"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@indent"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@mediatype"/>
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@encoding"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@omit-xml-declaration"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@cdata-section-elements"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@replace"/>
            <xsl:with-param name="default">"all"</xsl:with-param>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@instance"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@separator"/>
            <xsl:with-param name="default">"&amp;"</xsl:with-param>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@includenamespaceprefixes"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@validate"/>
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@ajx:synchronized"/>
            <xsl:with-param name="default">true</xsl:with-param>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@serialization"/>
        </xsl:call-template>
        <xsl:text>)</xsl:text>
        <xsl:for-each select="xforms:header">
            ...
            <xsl:text>,</xsl:text>
            <xsl:call-template name="toScriptParam">
                <xsl:with-param name="p" select="@combine"/>
                <xsl:with-param name="default">"append"</xsl:with-param>
            </xsl:call-template>
            <xsl:text>,</xsl:text>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:call-template name="toScriptParam">
                        <xsl:with-param name="p" select="xforms:name"/>
                    </xsl:call-template>
                </xsl:otherwise>
            </xsl:choose>
            <xsl:text>,[</xsl:text>
    ...
    <xsl:template match="xforms:toggle" mode="script" priority="1">
        ...
        <xsl:choose>
            ...
            <xsl:when test="xforms:case/text()">
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="normalize-space(xforms:case/text())"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="toScriptParam">
                    <xsl:with-param name="p" select="@case"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
        <xsl:text>,</xsl:text>
...
-->
    <xsl:template name="listeners">
        ...
        <xsl:for-each select="xforms:setvalue|xforms:insert|xforms:load|xforms:delete|xforms:action|xforms:toggle|xforms:send|xforms:setfocus|xforms:dispatch|xforms:message|xforms:show|xforms:hide">
            <xsl:text>new Listener(document.getElementById("</xsl:text>
            <xsl:choose>
                <xsl:when test="@ev:observer">
                    <xsl:value-of select="@ev:observer"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$rid"/>
                </xsl:otherwise>
...
-->
    <xsl:template match="*" mode="xml2string">
        ...
        <xsl:choose>
            <xsl:when test="namespace::*">
                <xsl:for-each select="(namespace::* | @*/namespace::*)[name()!='xml']">
                    ...
                    <xsl:if test="(not($parent/namespace::*[name()=$prefix and . = $uri]) or $root) and (($element|$element//*|$element//@*)[namespace-uri()=$uri])"> xmlns<xsl:if test="$prefix">:<xsl:value-of select="$prefix"/>
                        </xsl:if>="<xsl:value-of select="$uri"/>"</xsl:if>
                </xsl:for-each>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:apply-templates select="@*" mode="xml2string"/>
        <xsl:choose>
            <xsl:when test="node()">&gt;<xsl:apply-templates select="node()" mode="xml2string">
                    <xsl:with-param name="root" select="false()"/>
                </xsl:apply-templates>&lt;/<xsl:value-of select="name()"/>&gt;</xsl:when>
